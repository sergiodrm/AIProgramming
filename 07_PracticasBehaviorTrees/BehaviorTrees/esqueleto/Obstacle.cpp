...
void CObstacle::LoadFromFile(std::vector<std::shared_ptr<CObstacle>>& obstacles_, const char* _filename)
{
    ...
    for (paramElem; paramElem; paramElem = paramElem->NextSiblingElement())
    {
        ...
        if (!strcmp(paramName, "obstacle"))
        {
            ...
            obstacles_.push_back(std::make_shared<CObstacle>(pos, radius));
        }
    }
}

CObstacle::CObstacle(const USVec2D& _position, float _radius)
    : m_position(_position), m_radius(_radius), m_isColliding(false)
{ }
...
void CObstacle::DrawDebug() const
{
    ...
    MOAIDraw::DrawEllipseOutline(m_position.mX, m_position.mY, m_radius, m_radius, 10);
}
